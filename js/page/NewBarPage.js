...
export default class NewBarPage extends Component{
        ...
        return(
            <View>
                ...
                <Flex style={styles.inputContainer}>
                    ...
                    />
                    <Text style={this.state.text.length > 1 && !this.state.pending ? styles.inputSubmitYes:styles.inputSubmitNo}
    ...
    _sendMessage =() => {
        ...
        if(this.state.pictureList.length==0){
            ToastAndroid.show("请提供贴吧图像", ToastAndroid.SHORT);
            this.props.navigation.setParams({ pending: false });
            return null;
        }
        ...
        axios.put(url,
            {
                "name": text,
                "info": 'not avaliable now',
	            "avatar":pictureUriList[0]
            }
            ,{
            headers: {
            ...
            if(response.data.code == 0){
                ToastAndroid.show("申请成功" , ToastAndroid.SHORT);
                //this._onRefresh();
